...
The tab adds three safety affordances on top of that write path. Before Save, each row is checked client-side against a minimal inlined mirror of the Host's definite-error rules (missing package name, unbalanced flow collections in a config) so the offending row is flagged without a round trip — advisory only, since the Host-side rejection stays authoritative. Read-only targets render an in-panel copy form (source preset select, required new id, optional name — upstream's CopyForm pattern) instead of any browser dialog; it drives `target.copy`, refreshes the roster on success, and keeps its values with the Host diagnostic on failure. A stale draft banner offers reopening the target as a fresh draft, and a save refused with `PRESET_DRAFT_CONFLICT` refreshes the authoritative snapshot so the divergence banner guides reload-and-retry against fresh CAS fields.
