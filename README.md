...
| [`packages/dsh-web-search-router`](packages/dsh-web-search-router) | 🌱 v0.1 · [spec #8](https://github.com/S2P2/dsh-lab/issues/8) (implemented, dogfooding) · [ADR 0002](docs/adr/0002-web-search-router-model-detection.md) | Model-conditional web search behind the native `web_search` tool: Codex search on Codex models, z.ai search on GLM, keyed Exa→Tavily then free DDG/SearXNG fallback chain. Per-search model read (initiator + request header), failure rotation with 429 cooldowns, Provenance Notes, shared dsh-codex OAuth document. Host-only; hermetic provider-seam tests. |
