...
| `packages/dsh-quota-bar` | 🌱 dogfooding | GLM Coding Plan quota bar: 5h/weekly/tools windows + reset times above the composer (dock slot), host-side fetch over a loopback route. Verdict + prototype on `proto/quota-bar-slots`. |
