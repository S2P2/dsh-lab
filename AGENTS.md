...
| [`packages/dsh-tool-catalog-lean`](packages/dsh-tool-catalog-lean) | 🌱 v0.1 · dogfooding | Lean stock tool catalog: description-only compression of stock tool descriptions through the system-prompt/assemble waterfall — same tools, same schemas, shorter curated strings, with per-preset config overrides. |
